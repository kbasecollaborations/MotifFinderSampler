...
use lib qw(/lib/meme-5.0.1/perl);
...
unless ($@) {
  Log::Log4perl::init('/share/meme-5.0.1/logging.conf');
  $logger = Log::Log4perl->get_logger('meme.service.utils');
}

my $service_invocation_log_dir = '/var/meme-5.0.1/LOGS';
my $tmpdir = '';
...
sub update_status {
  ...
  my $template = HTML::Template->new(filename => '/share/meme-5.0.1/job_status.tmpl');
