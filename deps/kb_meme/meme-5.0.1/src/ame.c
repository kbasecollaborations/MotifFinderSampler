...
// Options struct.
typedef struct  {
  ALPH_T *alphapbet; // Alphabet; not really an option
...
void ame_set_defaults() {
  ...
  default_options.tsv_output = NULL;
...
static void ame_read_sequences(
  ...
  if (j == 0) die("All %ssequences are shorter than the longest motif (%d).\n", max_width);

  if (j != *num_seqs) {
    DEBUG_FMT(NORMAL_VERBOSE, "Warning: Removed %d primary sequence(s) that were shorter than the longest motif (%d)\n", 
...
void ame_terminate(int status) {
  ...
  if (!status && options.seq_source) final_print_results();
  exit(status);
} // ame_terminate
...
void ame_usage(char *format, ...) {
  ...
  char *usage = 
    ...
    "     --kmer <k>                preserve k-mer frequencies when shuffling letters; default: %d\n"
    ...
    "                                 default: avg\n"
    "     --hit-lo-fraction         <fraction> fraction of maximum log-odds for a hit; default: %g\n"
    "     --evalue-report-threshold <ev> motif significance reporting threshold; default: %g\n"
    ...
    "     --poslist                 [fasta|pwm] partition on affinity (fasta) or motif (pwm) scores;\n"
    "                                 default: fasta\n"
    ...
    "     --motif-pseudo            <pc> pseudocount for creating PWMs from motifs; default: %g\n"
...
void ame_getopt(int argc, char *argv[]) {
  ...
  while(simple_getopt(&option_name, &option_value, &option_index) > 0) {
    ...
    } else if (strcmp(option_name, "inc") == 0) {
...
AME_RESULT_T *ame_do_pearson_test(
  ...
  for (i=min; i <= max; i++) {
    ...
    if (options.linreg_switchxy) {
      if (use_ranks) {
        rho = pearson_correlation(i, y, x, NULL, NULL, NULL, &log_pv);
      } else {
        rho = pearson_correlation(i, y, x, &m, &b, &mse, &log_pv);
      }
    } else {
      if (use_ranks) {
	rho = pearson_correlation(i, x, y, NULL, NULL, NULL, &log_pv);
      } else {
	rho = pearson_correlation(i, x, y, &m, &b, &mse, &log_pv);
      }
    }
...
AME_RESULT_T **compute_motif_enrichment(
  ...
  *max_median_sequence_score = -1e300;
  ...
  for (db_i = 0, i = 1; db_i < arraylst_size(options.motif_sources); db_i++) {
    ...
    int motif_i = 0;				// index of motif within this DB
...
int main(int argc, char *argv[]) {
  ...
  DEBUG_FMT(NORMAL_VERBOSE, "Adjusted p-value threshold for reporting results: %g\n", options.evalue_report_threshold);
  ...
  ame_terminate(0); 	//Successfully end.
