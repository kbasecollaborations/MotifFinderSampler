...
)
{
  ...
  if (bfile) {				// -bfile given
    ...
    if (old_order != -1 && old_order != order) {
      fprintf(stderr, "WARNING: background order %d requested by the given file only contains order %d.\n",
	order, old_order);
    }
  } else if (order != -1) {		// -markov_order given
    // use fasta-get-markov to get model of desired order
    char cmd[100]; 
    char template[100]; sprintf(template, "/tmp/bfile.XXXXXX");
    tmp_bfile = mktemp(template);
    int argc = 0;
    char *argv[20];
    ...
    if (strcmp(alph_str, "DNA") == 0) argv[argc++] = "-dna";
    if (strcmp(alph_str, "RNA") == 0) argv[argc++] = "-rna";
    if (strcmp(alph_str, "PROTEIN") == 0) argv[argc++] = "-protein";
    if (! rc) argv[argc++] = "-norc";
    argv[argc++] = "-nosummary";
    argv[argc++] = "-nostatus";
    argv[argc++] = seqfile;
    argv[argc++] = tmp_bfile;
    fasta_get_markov(argc, argv);
    //sprintf(cmd, "echo tmp_bfile=%s; cat %s\n", tmp_bfile, tmp_bfile); system(cmd);
    back = load_markov_model(dataset->alph, &order, tmp_bfile);
    sprintf(cmd, "rm -f %s\n", tmp_bfile); 
