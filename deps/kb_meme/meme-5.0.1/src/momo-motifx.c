...
void find_most_significant_within_matrix(MATRIX_T* binomial,
  ...
  for (i = 0; i < get_num_rows(binomial); ++i) {
    for (j = 0; j < get_num_cols(binomial); ++j) {
      ...
      if (i != options->width/2 && !isnan(binomial_value) && !isinf(binomial_value) && binomial_value < log(options->score_threshold) && count_value >= options->min_occurrences) {
...
MATRIX_T* add_to_pattern(char* pattern,
  ...
  int col_idx = -1;
  double minimum = NAN;
  find_most_significant_within_matrix(binomial, phospho_count, &row_idx, &col_idx, &minimum, summary, options);
