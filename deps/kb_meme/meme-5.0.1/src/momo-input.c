...
void read_protein_database_sequences(MOMO_OPTIONS_T* options,
  ...
  if (protein_database_filename) {
    ...
    if (is_fasta) {
      read_many_fastas(alph, fp, MAX_SEQ, num_sequences, all_sequences);
    } else { // else Prealigned
      read_many_line_delimited_sequences(alph, fp, MAX_SEQ, num_sequences, options->width, all_sequences);
    }
    fclose(fp);
    options->bg_filetype = is_fasta ? Fasta : Prealigned;
...
void add_phospho_file_to_table(char* phospho_filename,
  ...
  while ((read = getline_nonempty(&line, &len, fp)) != -1) {
    ...
    bool passes_threshold = options->filetype == Prealigned || check_filter_threshold(filter_value, options);
    ...
    } else { // filetype = Prealigned or Fasta
  ...
  if (!found_mod && options->filetype == Psm) {
    fprintf(stderr, 
      ...
      "Refer to http://meme-suite.org/doc/psm-format.html for allowable formats.\n",
	phospho_filename);
    exit(EXIT_FAILURE);
  }
}
