...
static char* get_usage_message(
  ...
  if (algorithm == Simple) {
    ...
    sprintf(msg2, 
      //"     --count-threshold [positive integer] (default: %d)\n"
      "     --score-threshold [positive value] (default: %.1e)\n"
      "     --harvard (default: compute binomial CDF correctly)\n",
      //"     --printp print p-values for debugging (hidden option)\n",
...
static MOMO_OPTIONS_T process_momo_command_line(
  ...
  if ((algorithm == Motifx || algorithm == Modl) && ! options.protein_database_filename) {
    die("You must use option --protein-database when you specify the motifx or modl algorithms!");
  }
