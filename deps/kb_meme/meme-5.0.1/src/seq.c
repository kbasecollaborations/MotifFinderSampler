...
SEQ_T *shuffle_seq (
  ...
  // Create the new name, shortening if necessary.
  int pad = log(i_copy)/log(10) + 1;	// number of digits in the copy number
  char name[MAX_SEQ_NAME+1];
  int len = strlen(seq->name);
  strncpy(name, seq->name, MAX_SEQ_NAME);
  if (len > MAX_SEQ_NAME - pad) {
    len = MAX_SEQ_NAME - pad;
    name[len] = '\0';
  }
  sprintf(new_seq->name, "%s_shuf_%-3i", seq->name, i_copy);
