...
)
{
  ...
  *output_dirname = default_output_dirname;
  ...
  DO_STANDARD_COMMAND_LINE(1,
    ...
    NON_SWITCH(1, <dataset> \t\tfile containing sequences in FASTA format\n,
     ...
     DATA_OPTN(1, objfun, classic|de|se|cd|ce, \tobjective function (default: classic),
       obj = _OPTION_);
     DATA_OPTN(EXP, objfun, classic|de|se|ce|cd|nc|nz, \tobjective function (default: classic),
       obj = _OPTION_);
     ...
     DATA_OPTN(EXP, heapsize, <hs>,
               \tsize of heaps for widths where substring \n\t\t\t\tsearch occurs, 
               main_hs = atoi(_OPTION_));
     FLAG_OPTN(EXP, x_branch, \t\tperform x-branching, x_branch=true);
     FLAG_OPTN(EXP, no_x_branch, \t\tdo not perform x-branching, 
               no_x_branch=true);
     FLAG_OPTN(EXP, w_branch, \t\tperform width branching, w_branch=true);
     DATA_OPTN(EXP, bfactor, <bf>,
     ...
     FLAG_OPTN(EXP, print_heaps, \t\tprint heaps after each branching round,
       print_heaps = true);
     DATA_OPTN(1, brief, <n>, \t\tomit sites and sequence tables in .txt\n\t\t\t\toutput if more than <n> primary sequences,
       brief = atoi(_OPTION_));
  ...
  if (strcmp(alpha, "DNA") == 0) {
    ...
    fprintf(stderr, "Unknown alphabet type '%s'.\n", alpha);
    exit(1);
  }
  ...
  if (mtype != Tcm && dataset->n_samples < 2) {
    fprintf(stderr, "Your (primary) dataset must contain at least 2 sequences.\n");
    exit(1);
  }
  ...
  if (search_size == 0) {
    ...
    if (!NO_STATUS) fprintf(stderr, "Setting '-searchsize' and 'maxwords' to the actual size of primary sequences: %d\n", search_size);
  } else {
  ...
  init_meme_background(bfile, revcomp, dataset, alph_file, alpha, markov_order, datafile, 1);
  // Note: use primary sequences to insure that both background models are the
  // same or spurious motifs will be found.
  if (neg_dataset) init_meme_background(bfile, revcomp, neg_dataset, alph_file, alpha, markov_order, datafile, 0);  // not negfile!
  ...
  if (mtype != Tcm && (objfun == Classic || objfun == NC) && search_size_nseqs > classic_max_nsites) {
    search_size_nseqs = classic_max_nsites;
  }
  ...
  //if (!NO_STATUS)fprintf(stderr, "EM: search_size %d nseqs = %d search_size_nseqs %d\n", search_size, nseqs, search_size_nseqs);
  ...
  if (nsites != 0) {
    if (mtype == Oops) {
      fprintf(stderr, "You may not specify option '-sites' with option '-mod oops'.\n");
      exit(1);
    }
