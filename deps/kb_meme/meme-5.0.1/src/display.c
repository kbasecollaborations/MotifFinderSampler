...
static void print_log_odds(
  ...
  bool pair,          // double matrix if true 
...
static char *get_single_letter_consensus(
  MODEL *model,
  ALPH_T *alphabet
);
...
)
{
  ...
  */
  ...
  los[imotif] = create_lo(dataset->alph, model->imotif, w, logodds, false, thresh);
  ...
  print_log_odds(
    model->imotif, consensus, dataset, w, false, logodds, thresh, model->logev, outfile
  );
  // destroy logodds matrix
  for (i = 0; i < w; i++) free(logodds[i]);
  free(logodds);
...
)
{
  ...
  print_meme_training_set_xml(dataset, neg_dataset, nmotifs, outfile);
